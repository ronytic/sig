<?php
function listadotabla($titulo,$datos,$enlaces=0,$modifica="",$elimina="",$ver="",$botones="",$adicional="",$identificador="",$target="_self"){
	
	if($identificador=="" || empty($identificador)){$identificador="id";}
	...
	<tr class="contenido">
		...
		if($enlaces==1){
			...
			if(!empty($ver)){
			?>
				<td><a href="<?php echo $ver;?>?<?php echo $identificador;?>=<?php echo $id;?>" class="boton ver"><img src="<?php echo $folder; ?>imagenes/iconos/ver.png" alt="Ver" title="Ver"></a></td>
			<?php
			}
			if(!empty($modifica)){
			?>
				<td><a href="<?php echo $modifica;?>?<?php echo $identificador;?>=<?php echo $id;?>" class="boton modificar"><img src="<?php echo $folder; ?>imagenes/iconos/editar.png" alt="Modificar" title="Modificar"></a></td>
			<?php
			}
			if(!empty($elimina)){
			?>
				<td><a href="<?php echo $elimina;?>?<?php echo $identificador;?>=<?php echo $id;?>" class="boton eliminar"><img src="<?php echo $folder; ?>imagenes/iconos/cancelar.png" alt="Eliminar" title="Eliminar"></a></td>
			<?php
			}
			if(!empty($botones)){
			?>
				<?php foreach ($botones as $k => $v): ?>
				<td><a href="<?php echo $v;?>?<?php if(!empty($adicional)){foreach($adicional as $ak=>$av){ echo $ak."=".$av."&";}}?><?php echo $identificador;?>=<?php echo $id;?>" class="boton" target="<?php echo $target?>"><?php echo $k; ?></a></td>	
				<?php endforeach ?>
			<?php
			}
		}
...
function archivo($nombrearchivo){
	...
	switch($datos){
		...
		default:{echo recortarTexto($nombrearchivo,100);}break;
	}
}
...
function fechaSep($fecha,$c="d"){
	return strftime("%".$c,strtotime($fecha));	
}
...
function campos($texto,$nombre,$tipo,$valores="",$autofocus=0,$adicional=array(),$valorseleccion="",$linea=0){
	...
	switch($tipo){
		...
		case "select":{?>
        	<select id="<?php echo $nombre;?>" name="<?php echo $nombre;?>" <?php echo $autofocus==1?'autofocus':'';?><?php foreach($adicional as $k=>$v){echo ' '.$k.'="'.$v.'"';}?>>
		...
		default:{
			if(!is_array($valores))
		?><input type="<?php echo $tipo;?>" id="<?php echo $nombre;?>" name="<?php echo $nombre;?>" <?php echo $autofocus==1?'autofocus':'';?><?php foreach($adicional as $k=>$v){echo ' '.$k.'="'.$v.'"';}?> placeholder="Ingrese su <?php echo $texto;?>" value="<?php echo is_array($valores)?$valores[$nombre]:$valores;?>"/><?php
		}break;		
	}
	?>
	<?php
...
function escribir($ancho,$texto,$estilo="",$alineacion="L",$tam=10,$borde=0,$alto=5){
	global $pdf;
	$pdf->SetFont("arial",$estilo,$tam);
	$pdf->Cell($ancho,$alto,utf8_decode($texto),$borde,0,$alineacion);
}
function escribirM($ancho,$texto,$estilo="",$alineacion="L",$tam=10,$borde=0,$alto=5){
	global $pdf;
	$pdf->SetFont("arial",$estilo,$tam);
	$pdf->MultiCell($ancho,$alto,utf8_decode($texto),$borde,$alineacion,0);
}
?>