...
//		return $this->validateOperation ();
	...
	function mostrarTodo($where='',$orden=false,$cantidad=0,$ordendes=0){
		$this->campos=array('*');
		$orden=$orden?$orden:"id".$this->tabla;
		$condicion=$where?$where.' and ':'';
		if($cantidad==0)
			return $this->getRecords($condicion."activo=1",$orden,0,0,0,$ordendes);
		else
			return $this->getRecords($condicion."activo=1",$orden,0,$cantidad,0,$ordendes);
	}
...
//		echo "asd".$orden;
	...
	function actualizar($values,$Cod){
		$this->updateRow($values,"id".$this->tabla."=$Cod");	
	}
