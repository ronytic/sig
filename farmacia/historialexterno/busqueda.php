...
if (!empty($_POST)) {
	...
	foreach(${$a1nombre}->mostrarTodo("nombre LIKE '%$nombre%' and apep LIKE '%$apep%' and apem LIKE '%$apem%' and ci LIKE '%$ci%' ") as $pac){
		foreach(${$anombre}->mostrarTodo("idpaciente LIKE '%{$pac[idpaciente]}' and idpaciente LIKE '%{$idhistorialexterno}%' and fechaelaboracion LIKE '%$fechaelaboracion%'","fechaelaboracion",0,1) as $he){
			$i++;
...
//	echo $pa;
	...
	listadoTabla($titulo,$datos,1,"","","ver.php",array("Compra"=>"compra.php"),"","idhistorialexterno");
}
?>